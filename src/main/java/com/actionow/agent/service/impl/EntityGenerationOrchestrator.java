...
import com.actionow.agent.client.TaskLocalClient;
...
public class EntityGenerationOrchestrator implements EntityGenerationFacade {
    ...
    private final TaskLocalClient taskLocalClient;
    ...
    public EntityGenerationResponse retryGeneration(
        ...
        Result<Void> statusResult = assetLocalClient.updateGenerationStatus(workspaceId, assetId, "GENERATING");
        if (!statusResult.isSuccess()) {
            log.warn("更新素材状态失败: {}", statusResult.getMessage());
        }

        // 6. 更新 extraInfo
        Result<Void> extraInfoResult = assetLocalClient.updateAssetExtraInfo(workspaceId, assetId, extraInfo);
        if (!extraInfoResult.isSuccess()) {
    ...
    public Map<String, Object> getGenerationStatus(String assetId, String workspaceId) {
        ...
        if (StringUtils.hasText(taskId)) {
            Result<Map<String, Object>> taskResult = taskLocalClient.getTaskResult(taskId);
            if (taskResult.isSuccess()) {
                Map<String, Object> taskData = taskResult.getData();
    ...
    private Result<Map<String, Object>> createAsset(EntityGenerationRequest request,
        ...
        return assetLocalClient.createAsset(workspaceId, userId, assetRequest);
    }

    /**
     * 当 request.scriptId 缺失时，按 entityType 调用对应的 ProjectLocalClient 获取该实体的所属剧本。
    ...
    private String resolveScriptIdFromEntity(String entityType, String entityId) {
        ...
        try {
            Result<Map<String, Object>> result = switch (upper) {
                case "STORYBOARD" -> projectLocalClient.getStoryboard(entityId);
                case "EPISODE"    -> projectLocalClient.getEpisode(entityId);
                case "CHARACTER"  -> projectLocalClient.getCharacter(entityId);
                case "SCENE"      -> projectLocalClient.getScene(entityId);
                case "PROP"       -> projectLocalClient.getProp(entityId);
                case "STYLE"      -> projectLocalClient.getStyle(entityId);
                default -> null;
            };
    ...
    private Result<Map<String, Object>> createRelation(String entityType, String entityId,
        ...
        return assetLocalClient.createEntityAssetRelation(workspaceId, userId, relationRequest);
    }
    ...
    private Result<Void> updateAssetExtraInfo(String assetId, Map<String, Object> generationParams,
        ...
        return assetLocalClient.updateAssetExtraInfo(workspaceId, assetId, extraInfo);
    }
    ...
    private Result<Map<String, Object>> submitTask(String assetId, EntityGenerationRequest request,
        ...
        return taskLocalClient.submitAiGeneration(workspaceId, userId, taskRequest);
    }

    private void markAssetFailed(String assetId, String errorMessage, String workspaceId) {
        try {
            // 更新状态为 FAILED
            assetLocalClient.updateGenerationStatus(workspaceId, assetId, "FAILED");
