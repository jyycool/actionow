...
public class PollingTaskScanner {
    ...
    private void processOne(Task task) {
        ...
        try {
            statusResult = aiLocalClient.getExecutionStatus(executionId);
        } catch (Exception e) {
        ...
        try {
            resultData = aiLocalClient.getExecutionResult(executionId, 10);
        } catch (Exception e) {
            log.error("[PollingScanner] 获取执行结果 Local 调用失败: taskId={}, executionId={}, error={}",
    ...
    /**
     * 从 Task 实体恢复租户上下文
     * 供后续调用 AssetLocalClient、WalletLocalClient 等需要租户 Schema 的 Local 调用使用
     */
