...
public class StreamGenerationServiceImpl implements StreamGenerationService {
    ...
    public Flux<StreamGenerationEvent> streamGenerate(StreamGenerationRequest request,
        ...
        // 2. 获取提供商费用并冻结积分
        Result<AvailableProviderResponse> providerResult = aiLocalClient.getProviderDetail(request.getProviderId());
        ...
        if (creditCost > 0) {
            FreezeRequest freezeReq = FreezeRequest.builder()
                    ...
                    .remark("流式生成任务")
                    .build();
            Result<FreezeResponse> freezeResult = walletLocalClient.freeze(workspaceId, freezeReq);
    ...
    private long estimateCreditCost(String providerId, Map<String, Object> params,
                                     AvailableProviderResponse provider) {
        try {
            Result<Map<String, Object>> estimateResult = aiLocalClient.estimateCost(providerId, params);
