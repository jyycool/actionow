...
import com.actionow.task.client.WalletLocalClient;
...
public class PointsTransactionManagerImpl implements PointsTransactionManager {
    ...
    public String freezePoints(String workspaceId, String userId, Long amount,
        ...
        // 分布式锁：同一 businessId 串行化，防止并发/Local 重试导致重复冻结
    ...
    private String doFreezePoints(String workspaceId, String userId, Long amount,
        ...
        try {
            result = walletLocalClient.freeze(workspaceId, request);
        } catch (Exception e) {
    ...
    /**
     * 从补偿任务中恢复用户上下文
     * 定时调度线程无 HTTP 请求上下文，需手动设置以便 LocalRequestInterceptor 正确传递请求头
    ...
    private void doUnfreezeOnly(String workspaceId, String userId,
        ...
        Result<Void> result = walletLocalClient.unfreeze(workspaceId, request);
        if (!result.isSuccess()) {
    ...
    private void doRefundQuota(String workspaceId, String userId, long amount) {
        Result<Boolean> result = walletLocalClient.refundQuota(workspaceId, userId, amount);
        if (!result.isSuccess()) {
    ...
    private void doConfirmConsume(String workspaceId, String userId,
        ...
        Result<Void> result = walletLocalClient.confirmConsume(workspaceId, request);
        if (!result.isSuccess()) {
    ...
    private boolean useMemberQuota(String workspaceId, String userId, long amount) {
        try {
            Result<Boolean> result = walletLocalClient.useQuota(workspaceId, userId, amount);
