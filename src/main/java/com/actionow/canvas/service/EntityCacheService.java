...
public class EntityCacheService {
    ...
    /**
     * 调用 Project 本地客户端批量获取
     */
    private Result<List<EntityInfo>> batchFetch(String entityType, List<String> ids) {
        return switch (entityType.toUpperCase()) {
            case "SCRIPT" -> projectLocalClient.batchGetScripts(ids);
            case "EPISODE" -> projectLocalClient.batchGetEpisodes(ids);
            case "STORYBOARD" -> projectLocalClient.batchGetStoryboards(ids);
            case "CHARACTER" -> projectLocalClient.batchGetCharacters(ids);
            case "SCENE" -> projectLocalClient.batchGetScenes(ids);
            case "PROP" -> projectLocalClient.batchGetProps(ids);
            case "ASSET" -> projectLocalClient.batchGetAssets(ids);
            default -> {
