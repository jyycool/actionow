 ...
 * WebSocket 认证拦截器
 * 在握手阶段验证用户身份
 * 通过 Local 调用 user 模块进行 Token 验证
 *
 * @author Actionow
 */
@Slf4j
@Component("collabWebSocketAuthInterceptor")
@RequiredArgsConstructor
public class WebSocketAuthInterceptor implements HandshakeInterceptor {

    private final UserLocalClient userLocalClient;
    private final WorkspaceLocalClient workspaceLocalClient;
    ...
    public boolean beforeHandshake(ServerHttpRequest request, ServerHttpResponse response,
        ...
        if (request instanceof ServletServerHttpRequest servletRequest) {
            ...
            try {
                ...
                Result<TokenValidateResponse> result = userLocalClient.validateToken(tokenRequest);
    ...
    private boolean validateWorkspaceMembership(String workspaceId, String userId) {
        try {
            Result<WorkspaceMembershipInfo> result = workspaceLocalClient.getMembership(workspaceId, userId);
            if (result == null || !result.isSuccess() || result.getData() == null) {
